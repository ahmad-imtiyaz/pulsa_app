<x-app-layout :title="'Detail Voucher'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('voucher.edit', $voucher) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Edit</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <p class="text-2xl font-bold text-gray-900 mt-1">Rp {{ number_format($voucher->harga_modal, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <p class="text-sm text-gray-500">Harga Jual</p>
                <p class="text-2xl font-bold text-gray-900 mt-1">Rp {{ number_format($voucher->harga_jual, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <p class="text-sm text-gray-500">Laba/Unit</p>
                <p class="text-2xl font-bold text-green-600 mt-1">Rp {{ number_format($voucher->laba_per_unit, 0, ',', '.') }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <p class="text-sm text-gray-500">Stok Awal</p>
                <p class="text-2xl font-bold text-gray-900 mt-1">{{ $voucher->stok }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <p class="text-sm text-gray-500">Stok Tersedia</p>
                <p class="text-2xl font-bold {{ $voucher->hitungStokTersedia() > 0 ? 'text-green-600' : 'text-red-600' }} mt-1">
                    {{ $voucher->hitungStokTersedia() }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                ...
                <a href="{{ route('voucher.transaksi.create', $voucher) }}" class="text-sm text-blue-600 hover:text-blue-800">Tambah Transaksi</a>
            </div>
