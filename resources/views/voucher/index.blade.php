<x-app-layout :title="'Voucher'">
    ...
    x-data="{
        ...
        async liveSearch() {
            const url = new URL('{{ route('voucher.index') }}', window.location.origin);
            ...
            const newContent = doc.querySelector('[data-voucher-list]');
            const currentContent = document.querySelector('[data-voucher-list]');
...
>
    ...
    @foreach ($vouchers as $voucher)
        <tr class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 text-gray-900">
                Rp {{ number_format($voucher->harga_modal, 0, ',', '.') }}
            </td>

            <td class="px-6 py-4 text-gray-900">
                Rp {{ number_format($voucher->harga_jual, 0, ',', '.') }}
            </td>
            ...
            <td class="px-6 py-4 font-medium text-green-600">
                Rp {{ number_format($voucher->laba_per_unit, 0, ',', '.') }}
            </td>
            ...
            <td class="px-6 py-4">
                <div class="flex items-center space-x-2">
                    ...
                    <form method="POST"
                          action="{{ route('voucher.destroy', $voucher) }}"
                          class="inline"
