        ...
        <form method="POST" action="{{ route('aksesoris.transaksi.store', $aksesoris) }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div>
                ...
                <select name="jenis" id="jenis" required
                    ...
                    <option value="pembelian" {{ old('jenis') === 'pembelian' ? 'selected' : '' }}>Pembelian Stok (Masuk)</option>
                    <option value="penjualan" {{ old('jenis') === 'penjualan' ? 'selected' : '' }}>Penjualan (Keluar)</option>
                </select>
                @error('jenis')
            ...
            <div>
                ...
                <input type="number" name="harga_modal" id="harga_modal" value="{{ old('harga_modal', $aksesoris->harga_modal) }}" required min="0" step="any"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                @error('harga_modal')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div id="penjualan-fields" class="hidden space-y-4">
                <div>
                    ...
                    <input type="number" name="harga_jual" id="harga_jual" value="{{ old('harga_jual', $aksesoris->harga_jual) }}" min="0" step="any"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('harga_jual')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
